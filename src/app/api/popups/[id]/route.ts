...
function extractId(req: Request, params?: Record<string, any>) {
  ...
  if (!Number.isFinite(id)) throw new LaravelHttpError(400, 'Invalid popup id');
  return id;
}
...
export async function DELETE(req: Request, ctx: { params?: Record<string, any> }) {
  try {
    ...
    const data = await laravelFetch(`/popups/${id}`, {
      method: 'DELETE',
      auth: true,
    });
    ...
    return NextResponse.json({ message: 'Failed to delete popup' }, { status: 500 });
  }
}
