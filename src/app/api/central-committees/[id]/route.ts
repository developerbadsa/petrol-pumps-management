...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const committee = await prisma.centralCommittee.findUnique({ where: { id: Number(id) } });
  if (!committee) {
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  const existing = await prisma.centralCommittee.findUnique({ where: { id: Number(id) } });
  if (!existing) {
    return NextResponse.json({ message: 'Not found' }, { status: 404 });
  }
  ...
  const committee = await prisma.centralCommittee.update({
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.centralCommittee.delete({ where: { id: existing.id } });
